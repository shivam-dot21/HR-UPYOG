...
public class FileSignatureValidator {
	...
	static {
		...
		FILE_SIGNATURES.put("jpg", new byte[] { (byte) 0xFF, (byte) 0xD8 });

		FILE_SIGNATURES.put("jpeg", new byte[] { (byte) 0xFF, (byte) 0xD8 });
	...
	public void validateSignature(MultipartFile file, String extension) {
		...
		try (InputStream is = file.getInputStream()) {
			...
			if (read < expectedSignature.length) {
				throw new CustomException("INVALID_FILE", "File header too small");
			}
			...
			for (int i = 0; i < expectedSignature.length; i++) {
				...
				if (fileHeader[i] != expectedSignature[i]) {
					...
					throw new CustomException("INVALID_FILE", "File signature mismatch");
				}
			}

		} catch (IOException e) {

			throw new CustomException("INVALID_FILE", "Unable to read file signature");
		}
	}
}
